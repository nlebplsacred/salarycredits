...
class FastPayPage extends StatefulWidget {
  final ApplicantDashboardBaseModel dashboardBaseModel;
  final ProductList product;

  const FastPayPage({required this.product, required this.dashboardBaseModel, super.key});

  @override
  State<FastPayPage> createState() => _FastPayPageState();
}

class _FastPayPageState extends State<FastPayPage> {
  ...
  void initState() {
    ...
    SharedPreferences.getInstance().then((prefs) {
      setState(() {
        ...
        getLoanEligibility(3000); //default loading
      });
    });
  }
              ...
              Container(
                ...
                child: const Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      Text("What is Fast Pay?", style: AppStyle.pageTitle),
                      SizedBox(height: 8),
                      Text(
                          "Fast Pay is the swift solution for immediate financial needs. It allows you to access up to 50% of your monthly salary at unprecedented speed. With Fast Pay, you can address unexpected expenses, seize time-sensitive opportunities, or simply manage your finances with greater flexibility.",
                      ...
                      SizedBox(height: 4),
                      Text("Submit your request for Fast Pay through our user-friendly platform.", style: AppStyle.textLabel),
                      SizedBox(height: 8),
                      ...
                      SizedBox(height: 4),
                      Text("Gain immediate access to 50% of your monthly salary.", style: AppStyle.textLabel),
                      SizedBox(height: 8),
                      Text("Flexible Repayment:", style: AppStyle.pageTitle2),
                      SizedBox(height: 4),
                      Text("Repay the advanced amount within a one-month window.", style: AppStyle.textLabel),
                      SizedBox(height: 8),
                      ...
                      SizedBox(height: 12),
                      Text("Fast Pay empowers you with financial agility, ensuring you have the means to navigate any financial situation with ease.",
                          style: AppStyle.textLabel),
                    ],
                  ),
                ),
      ...
      children: [
        Visibility(
            ...
            child: user.netPayableSalary! >= 8000
                ...
                : getMessageBox("Sorry, basic minimum eligibility of salary did not meet hence you can not put request for Fast Pay")),
  ...
  Container getPartialSlider(LoanEligibilityResponseModel model) {
    ...
    // if (loanEligibilityResponseModel.getStatusId == 1) {
    //   model.setMinLoanEligibility = productList.minLoanAmount;
    //   int divCount = int.parse((model.getMaxLoanEligibility - model.getMinLoanEligibility).round().toString());
    //
    //   if (model.getAvailableLoanLimit <= 15000) {
    //     customDivisions = 100;
    //   } else if (model.getAvailableLoanLimit <= 50000) {
    ...
    return model.getAvailableLoanLimit >= 3000
        ? Container(
              ...
              children: [
                    ...
                    Expanded(
                      child: SliderTheme(
                          ...
                          child: Padding(
                            ...
                            child: Slider(
                                // value: _value.toDouble() > model.maxLoanEligibility ? 3000 : _value.toDouble(),
                                // min: model.minLoanEligibility == 0.0 ? 3000 : model.minLoanEligibility,
                                // max: model.maxLoanEligibility == 0.0 ? 3001 : model.maxLoanEligibility,
                                value: _value.toDouble(),
                                min: productList.minLoanAmount,
                                max: model.getAvailableLoanLimit == 0.0 ? 3001 : model.getAvailableLoanLimit,
                                ...
                                onChanged: (double newValue) {
                                  setState(() {
                                    ...
                                    if (valueTemp > 3000 && valueTemp < model.getAvailableLoanLimit) {
                                      _value = valueTemp;
                                    } else {
                                    ...
                                    if (newValue.round() > model.getAvailableLoanLimit) {
                                      _value = model.getAvailableLoanLimit.toInt();
                                    }
                                  });
                                },
                ...
                Padding(
                  ...
                  child: Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      Text("INR ${productList.minLoanAmount.round()}", style: AppStyle.pageTitle2),
                      Text("INR ${model.getAvailableLoanLimit.round()}", style: AppStyle.pageTitle2),
                    ],
                  ),
                ),
