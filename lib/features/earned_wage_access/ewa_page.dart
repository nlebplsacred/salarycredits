...
import '../loan_personal_details/loan_personal_form_page.dart';
import '../loan_process_pending/loan_process_pending_page.dart';
import '../loan_process_status/loan_process_status_page.dart';

class EWAPage extends StatefulWidget {
  final ApplicantDashboardBaseModel dashboardBaseModel;
  final ProductList product;

  const EWAPage({required this.product, required this.dashboardBaseModel, super.key});

  @override
  State<EWAPage> createState() => _EWAPageState();
}

class _EWAPageState extends State<EWAPage> {
  ...
  bool isEligibilityLoading = false, customLoader = false;
  ...
  void initState() {
    ...
    setState(() {
      _value = productList.minLoanAmount.toInt();
    });
    ...
    SharedPreferences.getInstance().then((prefs) {
      setState(() {
        ...
        getLoanEligibility(1000); //default loading
      });
    });
  }
  ...
  getPromoCodeDetails(String promoCode) async {
    ...
    try {
      await loanHandler
          .getPromoCodeDetails(user.applicantId!, productList.applicationTypeId, user.employerId!, promoCode)
          .then((CouponDetailsModel result) {
        setState(() {
          isLoadingPromo = false;
          couponDetailsModel = result;
  ...
  //load available limits for EWA
  Future<LoanEligibilityResponseModel> getLoanEligibility2(double amount) async {
    LoanEligibilityRequestModel requestModel = LoanEligibilityRequestModel(user.applicantId!, productList.applicationTypeId, amount, 1);
    return loanHandler.getLoanEligibility(requestModel);
  }

  getLoanEligibility(double amount) async {
    LoanEligibilityRequestModel requestModel = LoanEligibilityRequestModel(user.applicantId!, productList.applicationTypeId, amount, 1);
              ...
              Container(
                ...
                child: const Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      Text("What is EWA?", style: AppStyle.pageTitle),
                      SizedBox(height: 8),
                      Text(
                          "EWA(Earned Wage Access) makes earned salary available to you in real time, giving you instant access to ready liquidity any time of the month. It helps you to better align your income and expenses, and without engaging in high-interest credit schemes.",
                          style: AppStyle.textLabel),
                      SizedBox(height: 8),
                      Text(
                          "It enables you to access a portion of your earned salary at any time before your payday, with the remainder being paid at the end of the pay period without disrupting the payroll. It is based on the amount of work that you have completed for the period or the time elapsed at work.",
                          style: AppStyle.textLabel),
                      SizedBox(height: 8),
                      ...
                      SizedBox(height: 4),
                      Text("Submit your request for EWA through our user-friendly platform.", style: AppStyle.textLabel),
                      SizedBox(height: 8),
                      ...
                      SizedBox(height: 4),
                      Text("Gain immediate access to 80% of your earned wages.", style: AppStyle.textLabel),
                      SizedBox(height: 8),
                      Text("Repayment:", style: AppStyle.pageTitle2),
                      SizedBox(height: 4),
                      Text("Automatically adjusted from next salary payment", style: AppStyle.textLabel),
                      SizedBox(height: 8),
                      ...
                      SizedBox(height: 4),
                      Text("Enjoy the benefit of nominal transaction fee.", style: AppStyle.textLabel),
                      SizedBox(height: 8),
                      ...
                      SizedBox(height: 12),
                      Text("EWA empowers you with financial agility, ensuring you have the means to navigate any financial situation with ease.",
                          style: AppStyle.textLabel),
                    ],
                  ),
                ),
  ...
  Stack loadMainScreen(ActionTagDetails actionTagDetails) {
    ...
    return Stack(
      ...
      children: [
        Visibility(
            visible: true,
            child: user.netPayableSalary! >= 8000
                ? getMainSliderScreen(loanEligibilityResponseModel, actionTagDetails)
                : getMessageBox("Sorry, basic minimum eligibility of salary did not meet hence you can not put request for EWA")),
  ...
  Column getMainSliderScreen(LoanEligibilityResponseModel model, ActionTagDetails actionTagDetails) {
    ...
    if (actionTagDetails.statusId != 0) {
      ...
      if (actionTagDetails.statusId == 1) {
        ...
        textMessage = "Sorry, you are not eligible for EWA for now.";
      } else if (actionTagDetails.statusId != 6) {
        if (actionTagDetails.statusId == 3) {
          textMessage = "Due to recent rejection you can not apply an EWA for now.";
        }
      }
  ...
  Container getPartialSlider(LoanEligibilityResponseModel model) {
    int customDivisions = 100;

    return model.getMaxLoanEligibility >= 1000
        ? Container(
            ...
            child: Column(
              children: [
                Row(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
                    Text("INR $_value", style: AppStyle.sliderTitleAmt, textAlign: TextAlign.center),
                  ],
                ),
                      ...
                      child: SliderTheme(
                          ...
                          child: Padding(
                            ...
                            child: Slider(
                                ...
                                min: productList.minLoanAmount,
                                max: model.getMaxLoanEligibility == 0.0 ? 1001 : model.getMaxLoanEligibility,
                                activeColor: AppColor.lightBlue,
                                inactiveColor: AppColor.lightGrey,
                                thumbColor: AppColor.lightBlue,
                                divisions: customDivisions,
                                label: 'INR ${(_value / customDivisions).round() * customDivisions.round()}',
                                onChanged: (double newValue) {
                                  setState(() {
                                    int valueTemp = (newValue / customDivisions).round() * customDivisions;
                                    if (valueTemp > 1000 && valueTemp < model.getMaxLoanEligibility) {
                                      _value = valueTemp;
                                    } else {
                                      _value = newValue.round();
                                    }

                                    //limit the value as per total available limits
                                    if (newValue.round() > model.getMaxLoanEligibility) {
                                      _value = model.getMaxLoanEligibility.toInt();
                                    }
                                  });
                                },
                                onChangeEnd: (double newValue) {
                                  setState(() {
                                    getLoanEligibility(double.parse(_value.toString()));
                                  });
                                },
                ...
                Padding(
                  ...
                  child: Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      Text("INR ${model.getMinLoanEligibility.round()}", style: AppStyle.pageTitle2),
                      Text("INR ${model.getMaxLoanEligibility.round()}", style: AppStyle.pageTitle2),
                    ],
                  ),
                ),
                ...
                Row(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
                    Text("Repay from ${getMonthName(model.getEMIEndDate)} Salary", style: AppStyle.textLabel3),
                  ],
                ),
                Padding(
                  ...
                  child: Column(
                    children: [
                      ...
                      Row(
                        ...
                        children: [
                          const Text("Transaction Fee:", style: AppStyle.smallGrey),
                          Padding(
                      ...
                      Row(
                        mainAxisAlignment: MainAxisAlignment.end,
                        children: [
                          const Text("Total Interest Payable:", style: AppStyle.smallGrey),
                          Padding(
                            padding: const EdgeInsets.only(left: 16.0),
